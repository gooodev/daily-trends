...
**既出除外**: `site/src/lib/data/*.json`（リポジトリ内の既存の全公開ファイル）に含まれる過去の記事 URL（`categories[].items[].url`）をあらかじめ集め、今回の収集候補のうち URL が完全一致するものは除外する。過去に一度でも掲載した記事は、日付を問わず再掲しない。
...
- **過去に `site/src/lib/data/*.json` へ掲載済みの URL は手順 3 の既出除外で必ず取り除く**（同じ記事の再掲を防ぐ）
